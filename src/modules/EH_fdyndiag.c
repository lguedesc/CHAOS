...
void EH_fdyndiag(char *funcname, char* outputname, void (*edosys)(int, double *, double, double *, double *)) {
    ...
    EH_fdyndiag_read_params_and_IC(input_filename, &DIM, &nPar, &maxPer, &nP, &nDiv, &trans, &t, &par, &parRange, &indexX, &indexY, &x, &nRMS, &rmsindex, &dMode);
    ...
    EH_fdyndiag_print_info(output_info, DIM, nPar, maxPer, nP, nDiv, trans, t, x, par, parRange, indexX, indexY, nRMS, rmsindex, dMode, funcname, "screen");
    EH_fdyndiag_print_info(output_info, DIM, nPar, maxPer, nP, nDiv, trans, t, x, par, parRange, indexX, indexY, nRMS, rmsindex, dMode, funcname, "file");
    // To store the runtime of the program
    //double time_spent = 0.0;
    //clock_t time_i = clock();
    // Call solution
    EH_full_dynamical_diagram_solution(output_dyndiag, DIM, nP, nDiv, trans, maxPer, t, &x, indexX, indexY, parRange, par, nPar, nRMS, rmsindex, edosys, dMode, EH_p_write_fdyndiag_results);
...
void EH_fdyndiag_read_params_and_IC(char *name, int *dim, int *npar, int *maxper,  int *np, int *ndiv, int *trans, double *t, double **par, double **parrange, int *indexX, int *indexY, double **x, int *nrms, int **rmsindex, int *bifmode) {
    // Open input file
...
void EH_fdyndiag_print_info(FILE *info ,int dim, int npar, int maxper, int np, int ndiv, int trans, double t, double *x, double *par, double *parrange, int indexX, int indexY, int nrms, int *rmsindex, int bifmode, char* funcname, char* mode) {
    //Get time and date
    time_t tm;
    time(&tm);
