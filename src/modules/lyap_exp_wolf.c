...
void lyapunov_exp_wolf(char *funcname, void (*edosys)(int, double *, double, double *, double *)) {
    ...
    snprintf(output_lyap_name, sizeof(output_lyap_name), "%s%s_lyap", dir, funcname);       // Assign name for output rk4 without extension
    ...
    lyap_wolf_print_info(output_info, DIM, nPar, nP, nDiv, h, t, x, par, funcname, "screen");
    lyap_wolf_print_info(output_info, DIM, nPar, nP, nDiv, h, t, x, par, funcname, "file");
...
void lyap_wolf_print_info(FILE *info ,int dim, int npar, int np, int ndiv, double h, double t, double *x, double *par, char* edosys, char* mode) {
    ...
    if (strcmp(mode, "screen") == 0) {   
        printf("\n===================================\n");
        printf("Lyapunov Exponents (Wolf): %s\n", edosys);
        printf("===================================\n\n");
